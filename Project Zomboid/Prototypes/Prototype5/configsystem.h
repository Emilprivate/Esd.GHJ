...
struct Config
{
    std::string name;
    ImGuiStyle style;
    bool boolSettings[100]{ false };
	float floatSettings[100]{ 0.f };
	int intSettings[100]{ 0 };
...
void SaveConfig(Config& config)
{
    ...
    if (!filePath.empty())
    {
        ...
        if (file.is_open())
        {
            for (int i = 0; i < 15; ++i)
            {
                file << "settings[" << i << "]=" << (config.boolSettings[i] ? "true" : "false") << "\n";
            }
            ...
            file << "language=" << config.localizationManager.getCurrentLanguage() << "\n";
...
void LoadConfig(const std::string& name, Config& config)
{
    ...
    if (!filePath.empty())
    {
        ...
        if (file.is_open())
        {
            ...
            for (int i = 0; i < 15; ++i)
            {
                std::string key = "settings[" + std::to_string(i) + "]";
                auto it = configData.find(key);
                if (it != configData.end())
                {
                    if (it->second == "true")
                        config.boolSettings[i] = true;
                    else if (it->second == "false")
                        config.boolSettings[i] = false;
                }
            }
            ...
            auto it = configData.find("language");
            if (it != configData.end())
            {
                config.localizationManager.setLanguage(it->second);
            }
